...
8. **File structural shifts and 404s on raw URLs**: If you encounter 404s when attempting to fetch raw source files (e.g. standard entry points or key files), do not guess paths blindly. Use `web_search` with site-specific queries (e.g., `site:github.com/owner/repo "filename"`) or inspect the file tree API response to locate the exact path before requesting raw files. For example, some C projects structure their entry points under subdirectories (e.g., `src/cli/` instead of just `src/`).
