...
node publish.mjs publish                   # 发布到 ClawHub（自动 patch+1）
...
```
  Skill                     Current    Next
  ───────────────────────── ────────── ──────────
  anygen-suite              1.5.2      1.5.3

Commands:
  clawhub publish ".../anygen-suite" --slug "anygen-skill" --version "1.5.3"

  Continue? [y/N]
```

发布后验证：`clawhub inspect anygen-skill`
...
```bash
# 用户安装
npx skills add AnyGenIO/anygen-suite-skill
```
